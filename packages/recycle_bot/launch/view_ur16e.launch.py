...
def generate_launch_description():
    description_package = FindPackageShare("recycle_bot")
    description_file = PathJoinSubstitution(
        [description_package, "urdf", "recycle_bot_cell.urdf.xacro"]
    )
    rvizconfig_file = PathJoinSubstitution([description_package, "rviz", "urdf.rviz"])

    robot_description = ParameterValue(
        Command(["xacro ", description_file, " ", "ur_type:=", "ur16e"]), value_type=str
    )
